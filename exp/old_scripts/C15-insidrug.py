...
def main():
    ...
    files = glob.glob('/home/gustavo/ic2024/data/set2/C15/dimacs/c_*')

    for file in files:
        id = f"{file.split('/')[-1]}"
        exp_name = f"{id}"
        cmd = f"timeout 60s /home/gustavo/insidrug-mcqdw_v1.0.0/release/tools/misc/maxcliquedynweight --graph_file {file} --strategy maximum_weight_clique"
